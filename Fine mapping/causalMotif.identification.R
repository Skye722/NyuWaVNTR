...
# Load VNTR-SNP geno
file <- paste(argv[1],"-tss1MBSNP-motif.txt.gz",sep = "")
vntr_snp_geno <- read.csv(here::here("data/fineMap", file) ,header = TRUE,sep = ",")
vntr_snp_geno[1:7, 1:7]

# Load 543 VNTR-gene pairs
df.vntr_gene_pair <- read_csv(here::here("output/eVNTR",  "df.eMotif.FDR0.05.csv"))
...
genes <- sub("\\..*", "", genes)
intersect_genes <- intersect(vntr_snp_geno$geneId, genes)
#func
...
# fit
ofn <- paste( "/home/zhangsj/vntr/output/fineMap/",argv[1], paste(".pip.motif.", "L5", ".txt", sep=""), sep="")
fit_res <- lapply(1:length(intersect_genes), function(i) {
  print(i)
  g <- intersect_genes[i]
    # prep
